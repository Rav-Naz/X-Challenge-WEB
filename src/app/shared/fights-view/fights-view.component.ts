...
export class FightsViewComponent implements OnChanges {
  ...
  async editGroup() {
    if (this.selectedGroup) {
      this.isLoadingDeletingGroup = true;
      const nazwa = this.formManualNewGroup.get('group_name')?.value;
      this.groupsService.editGroup(this.selectedGroup, nazwa).then(resp => {
        this.ui.showFeedback("loading", "Zmieniono nazwe", 2)
      }, err => { console.log(err) }).finally(() => { this.isLoadingDeletingGroup = false; })
    }
  }
  ...
  get getPositionsForCategory() {
    ...
    if (positionsOptions && this.getSelectedCategory) {
      ...
      positionsOptions = positionsOptions.filter((pos: { kategorie: string; }) => pos.kategorie != null && pos.kategorie.split(', ').includes(this.getSelectedCategory!.toString()))
  ...
  async getBestRobotsInGroup() {
    ...
    let fights = [...this.getFightsInGroup];
    let robotsInGroup: Array<{
      robot: any;
      punkty: number;
    }> = [];
    robots.forEach(robot => {
      fights.forEach(fight => {
        if (fight.robot1_id == robot.robot.robot_id || fight.robot2_id == robot.robot.robot_id) {
          if (robotsInGroup.indexOf(robot) < 0) {
            robotsInGroup.push(robot);
          }
        }
      })
    });
    this.getFightsInGroup.forEach(fight => {
      if (fight.wygrane_rundy_robot1 != null && fight.wygrane_rundy_robot2 != null && fight.wygrane_rundy_robot1 > fight.wygrane_rundy_robot2) {
        let robot = robotsInGroup.find(r => r.robot.robot_id == fight.robot1_id);
        if (robot) robot.punkty += 1;
      }
      if (fight.wygrane_rundy_robot1 != null && fight.wygrane_rundy_robot2 != null && fight.wygrane_rundy_robot1 < fight.wygrane_rundy_robot2) {
        let robot = robotsInGroup.find(r => r.robot.robot_id == fight.robot2_id);
        if (robot) robot.punkty += 1;
      }
    })
