...
export class AuthService {
  ...
  async register(imie: string, nazwisko: string, email: string, kodPocztowy: string | null, numerTelefonu: string | null, rozmiarKoszulki: number, czyOpiekun: boolean, haslo: string, czyBedzieOsobiscie: boolean, wiek: number) {
    return new Promise<string>(async (resolve) => {
      ...
      const value = await this.http.register(imie, nazwisko, email, kod_pocztowy, numer_telefonu, rozmiarKoszulki, czy_opiekun, this.hashPassword(haslo).toString(), czyBedzieOsobiscie, wiek).catch(err => {
        if (err.status === 400) {
          this.errorService.showError(err.status, this.translate.instant(err.error.body));
        } else {
          this.errorService.showError(err.status);
