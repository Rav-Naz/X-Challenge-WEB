...
export class GroupsService {
  ...
  public editGroup(grupa_id: number, nazwa: string) {
    return new Promise<APIResponse | void>(async (resolve) => {
      const value = await this.http.editGroup(grupa_id, nazwa).catch(err => {
        if (err.status === 400) {
          this.errorService.showError(err.status, this.translate.instant(err.error.body));
        } else {
          this.errorService.showError(err.status);
  ...
  public WS_editGroup(data: any) {
    const index = this.groups.value?.findIndex(el => el.grupa_id == data.grupa_id)
    if (index != null && index != undefined && index >= 0) {
      this.groups.value![index].nazwa = data.nazwa;
      this.groups.next(this.groups.value);
    }
  }
